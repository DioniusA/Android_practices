...
class SignUpUseCase @Inject constructor(
    ...
    suspend operator fun invoke(email: String, password: String, confirmPassword: String): AppResult<User> {
        // Validate input
        if (email.isBlank()) {
        ...
        if (password != confirmPassword) {
            return AppResult.Error(AppError.InvalidCredentials("Passwords do not match"))
        }
